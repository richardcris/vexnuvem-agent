...
class GitHubUpdateService:
    ...
    def _build_upgrade_script(
        ...
        cmd_log_file = str(log_path.with_suffix(".cmd.log"))
        ...
        return (
            ...
            f"set \"CMD_LOG={cmd_log_file}\"\r\n"
            f"set \"RESTART_EXE={restart}\"\r\n"
            f"set \"INSTALL_DIR={install_target}\"\r\n"
            "\r\n"
            "echo [%DATE% %TIME%] Aguardando encerramento PID %TARGET_PID%... >> \"%CMD_LOG%\"\r\n"
            "\r\n"
            ":wait_for_app\r\n"
            "tasklist /FI \"PID eq %TARGET_PID%\" 2>nul | findstr /R /C:\" %TARGET_PID% \" >nul\r\n"
            "if errorlevel 1 goto pre_install\r\n"
            "powershell -Command \"Start-Sleep -Seconds 1\" >nul 2>nul\r\n"
            "goto wait_for_app\r\n"
            "\r\n"
            ":pre_install\r\n"
            "echo [%DATE% %TIME%] Processo encerrado. Aguardando 3s... >> \"%CMD_LOG%\"\r\n"
            "powershell -Command \"Start-Sleep -Seconds 3\" >nul 2>nul\r\n"
            "\r\n"
            ":run_installer\r\n"
            "if not exist \"%INSTALLER%\" (\r\n"
            "  echo [%DATE% %TIME%] ERRO: instalador nao encontrado: %INSTALLER% >> \"%CMD_LOG%\"\r\n"
            "  goto cleanup\r\n"
            ")\r\n"
            ...
            "echo [%DATE% %TIME%] ERRO: instalacao falhou. Ver log: %INSTALL_LOG% >> \"%CMD_LOG%\"\r\n"
            "goto cleanup\r\n"
            "\r\n"
            ":restart_app\r\n"
            "echo [%DATE% %TIME%] Instalacao OK. Reiniciando app... >> \"%CMD_LOG%\"\r\n"
            "powershell -Command \"Start-Sleep -Seconds 2\" >nul 2>nul\r\n"
            "if \"%RESTART_EXE%\" NEQ \"\" if exist \"%RESTART_EXE%\" (\r\n"
            "  echo [%DATE% %TIME%] Iniciando: %RESTART_EXE% >> \"%CMD_LOG%\"\r\n"
            "  start \"\" \"%RESTART_EXE%\"\r\n"
            ")\r\n"
            "\r\n"
            ":cleanup\r\n"
            "echo [%DATE% %TIME%] Fim do script de atualizacao. >> \"%CMD_LOG%\"\r\n"
